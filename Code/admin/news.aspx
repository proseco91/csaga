...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
        ...
        <div class="parenmenu_new">
            <div class="panel_from">
                <div class="panel_from_row panel_from_row_radio_btn">
                    <a href="tin-tuc.htm">
                        <input typeaction="0" type="button" value="Danh sách tin tức" /></a>
                    <a href="them-moi-tin-tuc.htm">
                        <input typeaction="1" type="button" value="Thêm mới tin tức" /></a>
                </div>
            </div>
        </div>
    </asp:Panel>
            ...
            <div class="style_table">
                ...
                %>
                    ...
                    <tr>
                        ...
                        <td style="width: calc(100% - 550px);" class="ellipsis" valdata="<%=item.value.TieuDe %>"><%=item.value.TieuDe %></td>
                        ...
                        <td style="width: 100px; text-align: center;" class="ellipsis">
                            <a href="tin-tuc-chinh-sua-<%=Lib.LocDau(item.value.TieuDe) %>-z-<%=item.value.ID %>.htm"><i class="fa fa-pencil" title="Chỉnh sửa"></i></a>
                            <a delete-link="Bạn chắc chắn muốn thay đổi trạng thái từ <%=item.value.Status==0?"ẩn thành hiển thị":"hiển thị thành ẩn" %> <%=item.value.TieuDe %>" href="tin-tuc-trangthai-<%=Lib.LocDau(item.value.TieuDe) %>-z-<%=item.value.ID %>.htm"><i class="fa fa-check-circle" title="<%=item.value.Status==1?"Ẩn":"Hiển thị" %>" style="<%=item.value.Status==1?"":"color:#CCC;" %>"></i></a>
                            <a delete-link="Bạn chắc chắn muốn xóa <%=item.value.TieuDe %>" href="tin-tuc-xoa-<%=Lib.LocDau(item.value.TieuDe) %>-z-<%=item.value.ID %>.htm"><i class="fa fa-times" title="Xóa"></i></a>
                        </td>
                    </tr>
                    <%}%>
                </table>
    ...
    <asp:Panel ID="add" runat="server" Visible="false">
        <div class="panel_from_item panel_from_item_show">
            <div class="panel_from_title"><span>Nội dung tin tức</span></div>
            <div style="clear: both; height: 20px;"></div>
            <div class="panel_from">
                <div style="clear: both; height: 20px;"></div>
