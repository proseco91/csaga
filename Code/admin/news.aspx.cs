...
public partial class admin_mat_hang : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Type != 0)
        {
            ...
            panelGioiThieu.Visible = false;
            panelImg.Visible = false;
            PanelMenu.Visible = false;
            menuActive = "1" + Request.QueryString["ID"];
        }
        else {
        ...
        else if (TypeAction == 1 || TypeAction == 2)
        {
            ...
            if (TypeAction == 2)
            {
                ...
                {
                    this.Title = "Chỉnh sửa '" + _data.TieuDe + "'";
                    if (!IsPostBack)
                    {
                        txtTitle.Text = _data.TieuDe;
                        txtChiTiet.Text = _data.Content;
                        txtDes.Text = _data.Des;
        ...
        else if (TypeAction == 3)
        {
            ...
            else
            {
                _data.Status = -1;
                sql.SubmitChanges();
                CreateMessage("Xóa '" + _data.TieuDe + "' thành công", true);
                Response.Redirect("mat-hang.htm");
            }
        ...
        else if (TypeAction == 4)
        {
            ...
            else
            {
                ...
                sql.SubmitChanges();
                CreateMessage("cập nhật trạng thái '" + _data.TieuDe + "' thành công", true);
            }
            Response.Redirect(Request.UrlReferrer.ToString());
        }
    }
