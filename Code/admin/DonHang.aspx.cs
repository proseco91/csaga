...
public partial class admin_EmailTemplate : BasePage
{
    ...
    public List<DonHang> getDonHang(int pageSize, out int totalRowCount, out int pageSelect)
    {
        int idWeb = Convert.ToInt32(Session["websiteID"]);
        int pageNum = 1;
        ...
        var query = sql.DonHangs.OrderBy(d=>d.Status).ThenByDescending(d=>d.CreateDate);
