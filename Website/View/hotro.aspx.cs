...
public partial class View_hotro : BaseHome
{
    ...
    public List<TinTuc> getThuVien(int pageSize, int cate, out int totalRowCount, out int pageSelect)
    {
        ...
        var query = sql.TinTucs.Where(d => d.Type == (int)Enums.LoaiTinTuc.HoTro && (d.Category.LastIndexOf(cate.ToString() + ",") > -1 || d.Category.LastIndexOf(cate.ToString()) > -1) && d.Status == (int)Enums.Status.active && (!d.ShowDate.HasValue || (d.ShowDate.HasValue && d.ShowDate.Value <= DateTime.Today)));
