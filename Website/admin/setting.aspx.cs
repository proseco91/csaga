...
public partial class admin_setting : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack) {
            ...
            Response.Write("<script type=\"text/javascript\">var ListImgOld = " + JsonConvert.SerializeObject(setting.Banner.Split(',').Where(d => !string.IsNullOrEmpty(d))) + ";</script>");
        }
    }
    ...
    public string getListImg(List<string> imgOld)
    {
        ...
        if (Request.Form["img_upload"] != null)
        {
            foreach (var base64 in Regex.Split(Request.Form["img_upload"], "-->end<--,"))
            {
                ...
                Lib.ResizeByWidth(new MemoryStream(Convert.FromBase64String(Regex.Replace(base64.Replace("-->end<--,", "").Replace("-->end<--", ""), "data:image/.*?;base64,", ""))), 1200).Save(serverMap + fileName, ImageFormat.Png);
